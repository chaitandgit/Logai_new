# Extract code and text from template or metadata.sample_first, and set mismatch flag
...
def check_mismatch(row):
    first = row.get('metadata.sample_first', None)
    last = row.get('metadata.sample_last', None)
...
# Extract code and text from template or metadata.sample_first, and set mismatch flag
def extract_code_and_text(row):
    # Try template columns first
    code = row.get('features.tpl_hash', None)
    text = row.get('features.tpl_text', None)
    # If not available, use metadata.sample_first
    if not code or not text:
        code = row.get('metadata.sample_first', None)
        text = row.get('metadata.sample_first', None)
    return code, text
...
def extract_code_and_text(row):
    ...
    if not code or not text:
        code = row.get('metadata.sample_first', None)
        text = row.get('metadata.sample_first', None)
# Extraction logic: code/text from template or sample, mismatch flag
def extract_code_and_text(row):
    code = row.get('features.tpl_hash', None)
    text = row.get('features.tpl_text', None)
    if not code or not text:
...
def extract_rfc5424_and_loglevel(msg):
    # Example: '2025-09-03T15:54:04.360606 L7 unbound: [17394:0] debug: new control connection ...'
...
if "RFC52424" not in df.columns or df["RFC52424"].isnull().all():
    ...
    df["RFC52424"] = df["RFC52424"].fillna("Unknown")
    if "log_level_text" in df.columns:
        df["log_level_text"] = df["log_level_text"].fillna("Unknown")
    else:
        if "message" in df.columns:
            _, df["log_level_text"] = zip(*df["message"].apply(lambda m: extract_rfc5424_and_loglevel(str(m))))
        else:
            df["log_level_text"] = "Unknown"


# 4. Ensure required schema
...
# If extracted_code was created, add to categorical_features
if "extracted_code" in df.columns and "extracted_code" not in categorical_features:
    categorical_features.append("extracted_code")



# Convert all timestamp columns and features.hour to CST
import pytz
from dateutil import parser


# Convert features.hour (if in ISO UTC) to CST hour
if "features.hour" in df.columns:
    def convert_hour_to_cst(val):
        try:
            dt = parser.isoparse(val)
            dt_cst = dt.astimezone(pytz.timezone("US/Central"))
            return dt_cst.hour
        except Exception:
            return None
    df["features.hour"] = df["features.hour"].apply(convert_hour_to_cst)

# Add missing columns and types
...
if X.empty:
    print("No data returned from Elasticsearch for the given query. Please adjust your time range or query parameters.")
    exit()
...
if "autoencoder" in cfg["ml_training"] and "params" in cfg["ml_training"]["autoencoder"]:
    contamination_value = cfg["ml_training"]["autoencoder"]["params"].get("contamination")
if contamination_value is None:
    contamination_value = cfg["ml_training"]["isolation_forest"]["params"].get("contamination", 0.05)
...
# Add RFC52424 to AutoEncoder features if present
if "RFC52424" in df.columns and "RFC52424" not in feature_cols_ae:
    feature_cols_ae.append("RFC52424")

# One-hot encode categorical features for autoencoder
from sklearn.preprocessing import OneHotEncoder
import numpy as np
cat_features_ae = [col for col in categorical_features if col in feature_cols_ae and col in df.columns]
num_features_ae = [col for col in feature_cols_ae if col not in cat_features_ae and col in df.columns]
for col in num_features_ae:
...
# Add human-readable explanation for each anomaly row
def explain_row(row):
    recon_err_cols = [c for c in df.columns if c.startswith("recon_err_")]
    if not recon_err_cols:
        return "No explanation available"
    # Find feature with highest reconstruction error
...
df["explanation_text"] = df.apply(lambda r: explain_row(r) if r["anomaly_label_autoencoder"] == 1 else "", axis=1)
...
# Convert iforest (-1=anomaly, 1=normal) and autoencoder (1=anomaly, 0=normal) to binary anomaly flags

# Convert all anomaly labels to 0 (normal) and 1 (anomaly)
...
df["anomaly_label_ensemble"] = ((df["anomaly_label_iforest"] + df["anomaly_label_autoencoder"]) >= 1).astype(int)  # Union (at least one flags anomaly)
# For strict majority (both must agree), use: ((df["anomaly_label_iforest"] + df["anomaly_label_autoencoder"]) == 2).astype(int)
...
output_cols = [col for col in df.columns if col not in cols_to_exclude]
